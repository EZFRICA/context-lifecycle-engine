...
#   uv run python examples/gdg_demo.py - topology competition demo (capture 1.000→0.600,
#                                           planted bridge false-trigger 0.143)
#   uv run python examples/make_vectors.py - regenerate the committed embedding cache
...
#     uv run examples/full_loop.sh
#
# Needs `uv` on PATH: every python and cle call below goes through `uv run`, so
# the script uses the project's environment wherever uv keeps it, instead of
# assuming it lives at `.venv/`.
#
...
cle() { uv run cle "$@" --state-dir "$STATE"; }
CLE=cle
...
# ---------------------------------------------------------------------------
# The whole offline suite - no real model, no API key, no network needed. Its
# size lives in docs/TESTING.md, where a guard keeps it true; nothing here would.
