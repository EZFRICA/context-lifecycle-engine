...
Suite: **384 tests across 42 files**, `python -m pytest -q`. Five more run only where the private WildChat corpus is present, so they are not counted here: a suite size a reader cannot reproduce is not a suite size. See
`docs/TESTING.md`.
